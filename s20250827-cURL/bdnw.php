...
/**
 * BNDW 近 30 年：每年最後一個交易日 Close / Adj Close、年報酬率、年化報酬率 (CAGR)
...
$ticker  = 'BNDW';                 // 目標：BNDW
$period1 = strtotime('2015-01-01'); // BNDW 2018 上市，抓早一點 OK
$period2 = time();

$yahooHistoryPage = 'https://finance.yahoo.com/quote/' . $ticker . '/history?p=' . $ticker;
...
$stooqUrl = 'https://stooq.com/q/d/l/?s=' . strtolower($ticker) . '.us&i=d'; // 備援：日線 CSV (無 Adj Close)
...
function fetch_yahoo_csv($historyPage, $downloadUrl)
{
    ...
    [$http1, $html, $err1] = curl_get_with_cookies($historyPage, $cookieFile);          // 先取 Cookie
    [$http2, $csv,  $err2] = curl_get_with_cookies($downloadUrl, $cookieFile, $historyPage); // 再下載 CSV
    @unlink($cookieFile);
...
// Yahoo 失敗 → 用 Stooq 備援（注意：無 Adj Close）
$useStooq = ($yHttp !== 200 || $yData === false || stripos($yData, 'Date,Open,High,Low,Close') !== 0);
$lastPerYear = [];
...
echo $ticker . " 近 30 年：每年最後交易日收盤價與報酬率（Close / Adj Close）\n";
echo str_pad("Year", 6) .
    str_pad("Date", 14) .
    str_pad("Close", 12) .
    str_pad("AdjClose", 12) .
...
// JSON 輸出
$jsonPath = __DIR__ . '/bndw_last_trading_day_30y.json';
$output = [
    'data' => $result,
    'cagr' => [
...
// CSV 輸出
$csvPath = __DIR__ . '/bndw_last_trading_day_30y.csv';
$fp = fopen($csvPath, 'w');
